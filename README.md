...
- 翻訳利用時:
  ...
  - Local LLM (Ollama): 起動済みのローカル server, `LOCAL_LLM_BASE_URL`, `LOCAL_LLM_MODEL`
...
OSS 配布向けの標準例は `Linux + Docker + Ollama` です。初回構成は `Ollama + qwen2.5:7b-instruct` を案内し、将来は `LOCAL_LLM_MODEL` と pull 対象を変えるだけで別モデルへ差し替えできます。

リポジトリ直下の [compose.yaml](/Users/watarunakamura/Desktop/zotero-annotator/compose.yaml) には Ollama 用 service が入っています。`ollama-data` volume を使うので、pull 済み model は再起動後も保持されます。

1. Ollama server を起動する

```bash
docker compose up -d ollama
```

2. 初回だけ標準 model を pull する

```bash
docker compose --profile init up ollama-pull
```

既に host 側で Ollama を動かしている場合は、代わりに次でも構いません。

```bash
ollama pull qwen2.5:7b-instruct
```

3. `.env` を local provider 用に切り替える
...
- `LOCAL_LLM_BASE_URL` は Ollama の OpenAI 互換 `/v1` endpoint を指してください。`http://localhost:11434` ではなく `http://localhost:11434/v1` を使います。
- `qwen2.5:7b-instruct` は README 上の標準例です。将来は別の Ollama model に置き換えて構いません。
- prompt 文面は ChatGPT backend と共通の [prompts.py](/Users/watarunakamura/Desktop/zotero-annotator/src/zotero_annotator/services/translators/prompts.py) を使います。

macOS は補足扱いです。Docker を使わない場合は Ollama app もしくは `ollama serve` で server を起動し、`ollama pull qwen2.5:7b-instruct` 実行後に同じ `.env` を設定してください。
