...
- Local LLM (Ollama): 起動済みのローカル server, `LOCAL_LLM_BASE_URL`, `LOCAL_LLM_MODEL`（詳細は [docs/local-llm.md](docs/local-llm.md)）
...
Local LLM を使う場合の `.env` 例と Ollama の起動手順は [docs/local-llm.md](docs/local-llm.md) を参照してください。
...
Local LLM (Ollama) を使う場合の詳細手順は [docs/local-llm.md](docs/local-llm.md) を参照してください。Docker 版と `macOS local` 版を分けて案内しており、macOS では通常 `Ollama app` / `ollama serve` を直接使う方が速いです。
